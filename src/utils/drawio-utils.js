 ...
 * @param {string} xmlString The potentially incomplete XML string
 * @returns {string} A legal XML string with properly closed tags and removed incomplete mxCell elements.
 */
 ...
 * @param {string} currentXML - The original Draw.io XML string
 * @param {string} nodes - The XML string containing new nodes to replace in the diagram
 * @returns {string} The updated XML string with replaced nodes
 */
    ...
    try {
        ...
        const hasCell0 = Array.from(nodesRoot.childNodes).some(
            node => node.nodeName === "mxCell" &&
                node.getAttribute("id") === "0"
        );

        const hasCell1 = Array.from(nodesRoot.childNodes).some(
            node => node.nodeName === "mxCell" &&
                node.getAttribute("id") === "1"
        );

        // Copy all child nodes from the nodes root to the current root, avoiding duplicates
 ...
 * @param {string} xmlContent - The original XML string
 * @param {Array<{ search: string, replace: string }>} searchReplacePairs - Array of search/replace objects
 * @returns {string} The updated XML string with replacements applied
 */
